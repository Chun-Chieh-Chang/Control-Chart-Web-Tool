...
var QIPExtractApp = {
  ...
  cacheElements: function () {
    this.els = {
      ...
      worksheetSelectGroup: document.getElementById(
        "qip-worksheet-select-group",
      ),
  ...
  bindEvents: function () {
    ...
    this.els.fileInput.addEventListener("change", function (e) {
      if (e.target.files.length > 0) self.loadFiles(e.target.files);
    });
    ...
    if (this.els.removeFile)
      this.els.removeFile.addEventListener("click", function () {
        self.removeFile();
      });
    if (this.els.resetFileBtn)
      this.els.resetFileBtn.addEventListener("click", function () {
        self.removeFile();
      });

    // Cavity count change
    if (this.els.cavityCount)
      this.els.cavityCount.addEventListener("change", function () {
        self.handleCavityCountChange();
      });

    // Worksheet selection
    if (this.els.worksheetSelect)
      this.els.worksheetSelect.addEventListener("change", function () {
        self.previewWorksheet();
      });
    if (this.els.previewBtn)
      this.els.previewBtn.addEventListener("click", function () {
        self.previewWorksheet();
      });

    // Sheet navigation
    if (this.els.prevSheet)
      this.els.prevSheet.addEventListener("click", function () {
        self.switchSheet(-1);
      });
    if (this.els.nextSheet)
      this.els.nextSheet.addEventListener("click", function () {
        self.switchSheet(1);
      });

    // Config management
    if (this.els.saveConfig)
      this.els.saveConfig.addEventListener("click", function () {
        self.saveConfiguration();
      });
    if (this.els.loadConfig)
      this.els.loadConfig.addEventListener("click", function () {
        self.showConfigDialog();
      });

    // Processing
    if (this.els.startProcess)
      this.els.startProcess.addEventListener("click", function () {
        self.startProcessing();
      });
    if (this.els.downloadExcel)
      this.els.downloadExcel.addEventListener("click", function () {
        self.downloadResults();
      });
    if (this.els.sendToSpc)
      this.els.sendToSpc.addEventListener("click", function () {
        self.sendToSPC();
      });
  },
  ...
  loadFiles: function (files) {
    ...
    if (validFiles.length === 0) {
      alert(
        this.t("請選擇有效的 Excel 檔案", "Please select valid Excel files"),
      );
      return;
    }
    ...
    Promise.all(promises)
      .then(function (results) {
        ...
        if (self.fileNames.length === 1) {
          ...
          self.els.fileName.textContent =
            self.fileNames.length + " " + self.t("個檔案...", "files...");
          self.els.fileName.title = self.fileNames.join("\n");
        }

        var totalSheets = self.workbooks.reduce(
          (acc, wb) => acc + wb.SheetNames.length,
          0,
        );
        self.els.workbookInfo.textContent =
          totalSheets +
          " " +
          self.t("個工作表 (來自 ", "Sheets (from ") +
          self.fileNames.length +
          " " +
          self.t("個檔案)", "files)");
        ...
        if (self.els.worksheetSelect.options.length > 1) {
          ...
          self.els.worksheetSelect.value =
            self.els.worksheetSelect.options[1].value;
          console.log(
            "QIP: Auto-previewing sheet:",
            self.els.worksheetSelect.value,
          );
          self.previewWorksheet();
          ...
          setTimeout(function () {
            self.els.previewPanel.scrollIntoView({
              behavior: "smooth",
              block: "start",
            });
          }, 300);
  ...
  renderCavityGroups: function (cavityCount) {
    ...
    for (var i = 1; i <= groupCount; i++) {
      ...
      html +=
        '<div class="qip-cavity-card p-4 bg-white dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700/50 space-y-3 shadow-sm hover:shadow-md transition-all">' +
        ...
        "</div>" +
        '<button class="qip-smart-select-btn px-3 py-1 bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white rounded-full text-[10px] font-bold shadow-sm transition-all flex items-center gap-1 group" data-group="' +
        i +
        '" title="' +
        this.t(
          "點選起點（Pt 1）與結束點（Pt 2），系統將自動以 Pt 1 所在行為 ID，其餘範圍為數據區域。",
          "Smart Select: Pick range including IDs and Data",
        ) +
        '">' +
        '<span class="material-icons-outlined text-xs">auto_awesome</span>' +
        "<span>" +
        this.t("智慧選取", "Smart Select") +
        "</span>" +
        "</button>" +
        "</div>" +
        '<div class="flex items-center justify-between">' +
        '<div id="qip-group-sheet-info-' +
        i +
        '" class="text-[10px] font-mono text-indigo-400 bg-indigo-50/30 dark:bg-indigo-900/10 px-2 py-0.5 rounded-full border border-indigo-100/50 dark:border-indigo-800/20">Sheet: -</div>' +
        "</div>" +
        ...
        '<div class="flex-1">' +
        '<label class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter block mb-0.5">' +
        this.t("穴號 ID 範圍", "Cavity ID Range") +
        "</label>" +
        '<input type="text" id="qip-cavity-id-' +
        i +
        '" class="qip-range-input w-full bg-transparent text-sm font-mono font-bold text-slate-700 dark:text-slate-300 border-b border-slate-100 dark:border-slate-700 focus:border-emerald-500 outline-none pb-1" placeholder="' +
        this.t("例如 K3:R3", "e.g. K3:R3") +
        '">' +
        "</div>" +
        "</div>" +
        // 數據範圍 (Data Values)
        ...
        '<div class="flex-1">' +
        '<label class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter block mb-0.5">' +
        this.t("數據讀值範圍", "Data Values Range") +
        "</label>" +
        '<input type="text" id="qip-data-range-' +
        i +
        '" class="qip-range-input w-full bg-transparent text-sm font-mono font-bold text-slate-700 dark:text-slate-300 border-b border-slate-100 dark:border-slate-700 focus:border-blue-500 outline-none pb-1" placeholder="' +
        this.t("例如 K4:R4", "e.g. K4:R4") +
        '">' +
        "</div>" +
        "</div>" +
        "</div>" +
    ...
    setTimeout(function () {
      document
        .querySelectorAll(".qip-smart-select-btn")
        .forEach(function (btn) {
          btn.addEventListener("click", function (e) {
            e.preventDefault();
            var groupIdx = this.dataset.group;
            // Trigger selection for the first field by default, but we'll handle multi-row logic in confirm
            self.startRangeSelection(
              "qip-cavity-id-" + groupIdx,
              "smart",
              groupIdx,
            );
          });
      ...
      document.querySelectorAll(".qip-range-input").forEach(function (input) {
        input.addEventListener("focus", function () {
          // Focus linking: Auto-trigger smart selection
          var card = this.closest(".qip-cavity-card");
          if (card) {
            var smartBtn = card.querySelector(".qip-smart-select-btn");
            if (smartBtn && !self.selectionTarget) {
              smartBtn.click();
            }
          }
        });
  ...
  renderPreviewTable: function (ws) {
    ...
    if (selectionTypeEl) {
      ...
      if (merges.length > 0) {
        selectionTypeEl.innerText =
          currentText +
          " (" +
          this.t("已偵測到 ", "Detected ") +
          merges.length +
          " " +
          this.t("個合併區域", "merges") +
          ")";
    ...
    var html =
      '<table class="qip-preview-table" style="border-collapse: collapse; font-size: 12px; width: 100%; color: var(--table-text); background: var(--table-bg);">';

    // Header row
    html +=
      '<thead><tr><th style="border: 1px solid var(--table-border); padding: 4px; background: var(--table-header-bg); width: 40px;"></th>';
    for (var c = 0; c < maxCols; c++) {
      html +=
        '<th style="border: 1px solid var(--table-border); padding: 4px; background: var(--table-header-bg); font-weight: bold; min-width: 60px;">' +
    ...
    for (var r = 0; r < maxRows; r++) {
      html +=
        '<tr><th style="border: 1px solid var(--table-border); padding: 4px; background: var(--table-header-bg); font-weight: bold;">' +
        (r + 1) +
        "</th>";
      for (var c = 0; c < maxCols; c++) {
      ...
      cell.addEventListener("click", function (e) {
        ...
        if (!self.firstClickCell) {
          ...
          if (self.els.selectionTypeText) {
            var originalText =
              self.selectionType === "cavity"
                ? self.t("穴號範圍", "Cavity ID Range")
                : self.t("數據範圍", "Data Range");
            self.els.selectionTypeText.textContent =
              originalText +
              " (" +
              self.t("請點選結束位置", "Select end point") +
              ")";
          }
        } else {
          // Second click: set the end point and confirm
          self.highlightRange(
            self.firstClickCell.row,
            self.firstClickCell.col,
            currentCell.row,
            currentCell.col,
          );
  ...
  startRangeSelection: function (targetId, type, groupIndex) {
    ...
    if (this.els.selectionModeIndic && this.els.selectionTypeText) {
      ...
      this.els.selectionTypeText.textContent =
        type === "cavity"
          ? this.t("穴號範圍", "Cavity ID Range")
          : this.t("數據範圍", "Data Range");
    }
    ...
    this.els.previewPanel.scrollIntoView({
      behavior: "smooth",
      block: "nearest",
    });
  },
  ...
  clearSelection: function () {
    ...
    confirmRangeSelection: function (start, end) {
        ...
        if (this.selectionType === 'smart' && sRow !== eRow && this.activeSelectionGroup) {
            ...
            var idRangeStr = idAddrS === idAddrE ? idAddrS : idAddrS + ':' + idAddrE;
            var dataRangeStr = dataAddrS === dataAddrE ? dataAddrS : dataAddrS + ':' + dataAddrE;

            var idInput = document.getElementById('qip-cavity-id-' + this.activeSelectionGroup);
            var dataInput = document.getElementById('qip-data-range-' + this.activeSelectionGroup);
            ...
            // Standard behavior or single row
    ...
    if (this.activeSelectionGroup) {
      ...
      var infoEl = document.getElementById(
        "qip-group-sheet-info-" + this.activeSelectionGroup,
      );
      if (infoEl) {
  ...
  gatherConfiguration: function () {
    ...
    for (var i = 1; i <= groupCount; i++) {
      var sheetIdx =
        this.groupSheetIndices[i] !== undefined
          ? this.groupSheetIndices[i]
          : i === 1
            ? worksheetSelect.selectedIndex - 1
            : 0;
      var sheetName = (worksheetSelect.options[sheetIdx + 1] || {}).value || "";

      config.cavityGroups[i] = {
        cavityIdRange:
          (document.getElementById("qip-cavity-id-" + i) || {}).value || "",
        dataRange:
          (document.getElementById("qip-data-range-" + i) || {}).value || "",
        pageOffset: sheetIdx,
  ...
  saveConfiguration: function () {
    ...
    if (!name) {
      alert(this.t("請輸入配置名稱", "Please enter configuration name"));
      return;
    }
  ...
  showConfigDialog: function () {
    ...
    var choice = prompt(
      this.t("請輸入欲讀取的配置編號 (0-", "Enter layout ID to load (0-") +
        (configs.length - 1) +
        "):" +
        "\n\n" +
        list,
  ...
  startProcessing: async function () {
    var self = this;
    if (!this.workbooks || this.workbooks.length === 0) {
      alert(this.t("請先上傳檔案", "Please upload files first"));
      return;
    }
    ...
    // Basic validation
    if (
      !config.cavityGroups[1] ||
      !config.cavityGroups[1].cavityIdRange ||
      !config.cavityGroups[1].dataRange
    ) {
      alert(
        this.t(
          "請至少設定第一組的穴號範圍和數據範圍",
          "Please set Cavity ID and Data range for Group 1",
        ),
      );
      return;
    }
    ...
    this.els.progressText.textContent = this.t(
      "初始化處理器...",
    ...
    try {
      ...
      // 處理多個工作簿
      this.processingResults = await processor.processWorkbooks(
        this.workbooks,
        (progress) => {
  ...
  showResults: function (results) {
    ...
    setTimeout(function () {
      ...
      if (self.els.progressText)
        self.els.progressText.textContent = "Extraction Complete";
    }, 100);
    ...
    var summary =
      '<div class="space-y-3">' +
      '<div class="flex justify-between items-center"><span class="text-sm text-slate-500 uppercase font-bold">' +
      this.t("產品資料", "Product") +
      '</span><span class="text-sm font-bold text-white truncate max-w-[150px]">' +
      (results.productInfo.productName || "-") +
      "</span></div>" +
      '<div class="flex justify-between items-center"><span class="text-sm text-slate-500 uppercase font-bold">' +
      this.t("提取項目數", "Extracted Items") +
      '</span><span class="text-sm font-bold text-emerald-400">' +
      results.itemCount +
      "</span></div>" +
      '<div class="flex justify-between items-center"><span class="text-sm text-slate-500 uppercase font-bold">' +
      this.t("總批次數", "Total Batches") +
      '</span><span class="text-sm font-bold text-indigo-400">' +
  ...
  renderExtractionDetail: function (results) {
    ...
    var detailContent = document.getElementById(
      "qip-extraction-detail-content",
    );
    ...
    if (itemNames.length === 0) {
      html =
        ...
        this.t("無提取數據", "No extracted data") +
        "</div>";
    } else {
      itemNames.forEach(function (itemName, idx) {
        ...
        // Header
        html +=
          '<div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" onclick="this.nextElementSibling.classList.toggle(\'hidden\')">';
        ...
        html +=
          '<span class="material-icons-outlined text-sm text-indigo-600">' +
          (idx % 3 === 0 ? "straighten" : idx % 3 === 1 ? "speed" : "widgets") +
          "</span>";
        html += "</div>";
        ...
        html +=
          '<span class="text-xs text-slate-500"><span class="font-mono font-bold text-indigo-600">' +
          totalBatches +
          "</span> " +
          (totalBatches > 1 ? "batches" : "batch") +
          "</span>";
        html += '<span class="text-xs text-slate-400">•</span>';
        html +=
          '<span class="text-xs text-slate-500"><span class="font-mono font-bold text-emerald-600">' +
          cavityCount +
          "</span> " +
          (cavityCount > 1 ? "cavities" : "cavity") +
          "</span>";
        html += "</div>";
        html += "</div>";
        html += "</div>";
        ...
        // Content (Collapsible)
        html +=
          '<div class="hidden border-t border-slate-200 dark:border-slate-700">';

        // Cavity Summary
        html +=
          '<div class="p-4 bg-slate-50/50 dark:bg-slate-900/20 border-b border-slate-200 dark:border-slate-700">';
        html +=
          '<div class="text-xs font-bold text-slate-500 uppercase mb-2">' +
          (window.currentLang === "zh" ? "穴號範圍" : "Cavity Range") +
          "</div>";
        html += '<div class="flex flex-wrap gap-1">';
        allCavities.forEach(function (cav) {
          html +=
            '<span class="px-1.5 py-0.5 bg-emerald-100 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-xs font-mono rounded">' +
            cav +
            "</span>";
        });
        html += "</div>";
        ...
        item.batches.slice(0, 5).forEach(function (batch) {
          ...
          var cavIds = Object.keys(batchData).sort(function (a, b) {
            return parseInt(a) - parseInt(b);
          });

          html +=
            '<div class="flex items-start gap-2 p-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">';
          html +=
            '<div class="text-xs font-mono font-bold text-slate-700 dark:text-slate-300 min-w-[80px]">' +
            batchName +
            "</div>";
        ...
        if (totalBatches > 5) {
          html +=
            ...
            (window.currentLang === "zh" ? "及其他 " : "and ") +
            (totalBatches - 5) +
            (window.currentLang === "zh" ? " 筆批次" : " more batches") +
            "</div>";
        }
        html += "</div>";
  ...
  downloadResults: async function () {
    if (!this.processingResults) {
      alert(this.t("沒有可下載的結果", "No results to download"));
      return;
    }

    try {
      const exporter = new ExcelExporter();
      exporter.createFromResults(
        this.processingResults,
        this.processingResults.productCode,
      );
  ...
  sendToSPC: function () {
    console.log(
      "QIP: Preparing to send data to SPC analysis...",
      this.processingResults,
    );
    ...
    if (!this.processingResults) {
      alert(
        this.t(
          "沒有可傳送的數據：請先點擊「開始提取數據」按鈕。",
          'No data to send: Please click "Run Extraction" first.',
        ),
      );
      return;
    }
    ...
    if (itemCount === 0) {
      alert(
        this.t(
          "沒有可傳送的數據：提取結果中不包含任何有效的檢驗項目。",
          "No data to send: No valid inspection items found.",
        ),
      );
      return;
    }
    ...
    if (typeof SPCApp !== "undefined") {
      ...
      alert(
        this.t(
          "數據已準備就緒，請在 QIP 匯入頁面選擇檢驗項目進行分析。",
          "Data ready. Please select an item on the Import page to analyze.",
        ),
      );
    } else {
      ...
      alert(
        this.t(
          "無法連接 SPC 分析模組",
          "Cannot connect to SPC analysis module",
        ),
      );
    }
  },

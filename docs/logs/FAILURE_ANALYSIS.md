...
### 1.3 代碼重構導致的 ReferenceError 與邏輯偏離

- **失敗現象**：
  1. 運行時報錯 `idRow is not defined`。
  2. 項目顯示順序與原始 Excel 不符，變為字母排序。
- **原因分析**：
  1. **誤刪變數**：在清理「過度干預邏輯」時，不慎將必要的座標變數 `idRow` 一併刪除。
  2. **對象排序特性**：使用 JavaScript Object 存儲數據導致解析順序隨瀏覽器默認排序（字母序），破壞了原始代碼的「物理順序」原則。
- **矯正措施**：
  1. **建立 Sequence 基準**：引入 `itemOrder` 數組紀錄原始掃描順序，解決 JS 對象無序問題。
  2. **加強代碼走讀**：在還原邏輯後執行全量 `grep` 檢查，確保所有依賴變數（如 `idRow`）在渲染路徑中均有正確定義。
  3. **原則確認**：在任何 AI 輔助開發中，必須將「介面優化」與「底層邏輯」解耦，介面輔助工具絕對不應改變底層數據讀取規則。

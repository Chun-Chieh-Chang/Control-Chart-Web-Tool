...
- **數據邏輯修正 (Data Logic Fix)**:
  - **允許同名批號重複 (Allow Duplicate Batch Names)**: 完全移除對批號的修改或合併邏輯。若多個檔案中包含完全相同的批號名稱 (如 `Setup`)，系統將會在報表中忠實呈現多次 (即顯示 `Setup`, `Setup`, ...)，代表不同的數據來源，而不會嘗試將其合併或更名。此邏輯適用於所有批號名稱 (如 `AAA`, `BBB` 等)。

...
- **資料顯示順序優化**: 修正了結果清單自動按字母排序的問題。現在嚴格遵循 **「物理掃描順序 (Sequential Scanning)」**：按照 Excel 工作表由左至右、數據行由上至下的順序紀錄並顯示項目，確保結果排列與原始報表結構完全一致。
- **穴號欄位完整性**: 解決了無數據穴號被跳過的問題。現在即便特定穴號在某批次中無數據，只要其位於選取的 ID 範圍內，系統仍會保留該穴號欄位，確保導出的 Excel 結構穩定且不跳碼。

...
## [2026-04-11] UI/UX Contrast & Accessibility Optimization

### 1. Issue Description

- **UI Contrast Deficiency**: Users reported difficulty reading text within the gray analytical model selection cards.
- **Cause**: Low-contrast background (`bg-slate-50/50`) combined with de-emphasized text (`text-slate-500`) created a "foggy" interface that failed accessibility standards in both light and dark modes.

### 2. Root Cause Analysis (RCA)

- **RCA-F: Material Design Over-minimalism**: The use of semi-transparent slate backgrounds (`/50`) intended for "minimalism" reduced the luminosity difference between the background and text below critical legibility thresholds.
- **RCA-G: Dynamic Component Over-standardization**: Inspection item cards were generated by JS with a fixed base style that didn't adapt to the premium elevated surface requirement for "Selected" states.

### 3. Corrective and Preventive Action (CAPA)

- **Premium Surface Implementation**:
  - Replaced all gray-on-gray cards with **Solid White surfaces** in Light Mode and **High-luminance Slate (800/40)** in Dark Mode.
  - Strengthened card borders to `border-2 border-slate-100` to define clear physical boundaries.
- **Typography Weight Adjustment**:
  - Upgraded description text from `slate-500` to `slate-600` (Light) and `slate-300` (Dark).
  - Hardened heading weights to ensure immediate visual hierarchy.
- **Interactive State Hardening**:
  - Optimized the "Active" state for inspection items in `app.js` using a thicker `ring-2` and higher-contrast border to ensure zero ambiguity in user selection.

### 4. Technical Achievements

- [x] **WCAG Compliance**: Drastically increased contrast ratios for all critical selection UI elements.
- [x] **Visual Consistency**: Synchronized static and dynamically generated cards to follow the "Premium Clean" design system.
- [x] **Responsive Polish**: Enhanced hover and active feedback for touch and mouse interactions.
...
- **View Slicing**: The Dashboard only displays a slice of the history (`.slice(0, 5)`). If a user deleted the first item on the dashboard (UI index 0), the application would remove the 0th item of the _entire_ history array, which might not be the same record if the list was sorted or filtered.
...
- **RCA-B (Numeric Axis Constraint)**: ApexCharts determines bar width based on the _minimum_ distance between X-points across all series. A high-res line series (200 pts) forces the bar series to shrink to a tiny fraction of the bin width.
...
- [x] **Accessibility Standard Compliance**: Enforced 14px minimum font size and WCAG AAA contrast levels for dark mode.

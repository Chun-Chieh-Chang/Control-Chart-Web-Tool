...
- **QIP 提取邏輯深度優化 (QIP Extraction Core Fix)**:
  - **支持多檔案規格提取 (Multi-Workbook Spec/Info)**: 重構提取引擎，使其能掃描所有上傳的 Excel 檔案以獲取規格與產品資訊，解決部分檔案資訊不完整導致的提取空值問題。
  - **智慧批次合併 (Smart Batch Merging)**:
    - **內部合併**: 同一檔案中具有相同基準名稱（移除 `-1`, `(2)` 等數字後綴）的工作表將自動合併為單一批次數據。
    - **外部隔離**: 不同檔案中的同名工作表（如 `Setup`）將被視為獨立批次，防止數據覆蓋，確保跨檔案分析的準確性。
